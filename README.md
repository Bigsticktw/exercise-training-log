...
8. 開啟本 App 的「同步設定」，貼上 URL 並儲存。可先用 Web App URL 的 `GET` 回應確認 `ok: true`。

注意：任何人可呼叫的 Web App 端點都應視為可公開存取；不要把密鑰或個資放進前端或 payload。若未來需要強身分驗證，應另建有驗證能力的後端，而非把共享密鑰寫在 JavaScript。
